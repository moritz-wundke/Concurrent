...
class Component(object):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if self is None:
            ...
            compmgr.activate_component(self)
        return self
...
class ComponentManager(object):
    ...
    def __getitem__(self, cls):
        ...
        existing instance if the component has already been activated and it's
        ...
        if not component or not component.is_singleton():
            if cls not in ComponentMeta._components:
                raise BaseError('Component "%s" not registered' % cls.__name__)
            try:
                component = cls(self)
            except TypeError as err:
                raise BaseError('Unable to instantiate component %r (%s)' %
                                (cls, err))
        return component
